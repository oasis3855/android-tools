...
インストールする実機に合わせてフォントサイズ、照度センサーのしきい値を変更しapkファイルを再作成する必要があります。 → Version 1.1 で設定画面追加、 Version 1.2で設定値をプロファイルに保存する機能
...
- 設定値をSharedPreferencesに保存
- 気象庁アメダスデータを読み込んで、気温・湿度・1H降水量の表示
...
- Android 4 (SSL未対応のため気象庁アメダス受信機能は動作しない)
- Android 5
- Android 7
- Android 10
- Android 11
...
- Version 1.2 (2025/06/22)
  ...
  - 設定値をプロファイル保存
  - 気象庁アメダス（気温、湿度、降水量）の表示

- Version 1.3 (2025/07/01)

  - ユーザ指定URLからJSONファイルをダウンロード
  - HTTP対応(Android 5/API 19以下はTLS 1.2までしか対応できないため. TLS 1.2有効化方法は fetchStations() のコメントアウト部分を参照)
